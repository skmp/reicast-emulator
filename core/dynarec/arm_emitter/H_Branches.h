...
namespace ARM
{
	...
	EAPI CALL(unat FnAddr, ConditionCode CC=AL)
	{
		...
		if(0==lit) {
			wprintf(L"Error, Compiler caught NULL literal, CALL(%08X)\n", FnAddr);
			verify(false);
			return;
		}
		if( (lit<-33554432) || (lit>33554428) )     // ..28 for BL ..30 for BLX
		{
			wprintf(L"Warning, CALL(%08X) is out of range for literal(%08X)\n", FnAddr, lit);
			// verify(false);
	...
	EAPI JUMP(unat FnAddr, ConditionCode CC=AL)
	{
		...
		/*if(0==lit) {
			wprintf(L"Error, Compiler caught NULL literal, JUMP(%08X)\n", FnAddr);
			verify(false);
			return;
		}*/
		if( (lit<-33554432) || (lit>33554428) )     // ..28 for BL ..30 for BLX
		{
			wprintf(L"Warning, %X is out of range for imm jump! \n", FnAddr);
			//verify(false);
