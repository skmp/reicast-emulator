...
void prof_periodical()
{
	...
	LOG_V("profiler", "TA_VTXC %d,TA_SPRC %d,TA_EOSC %d,TA_PPC %d,TA_SPC %d,TA_EOLC %d,TA_V64HC %d\n", TA_VTXC,TA_SPRC,TA_EOSC,TA_PPC,TA_SPC,TA_EOLC,TA_V64HC);
	...
	LOG_V("profiler", "Ret cache: %.2fM rets, %.2f:1 HRD, %.2f:1 HRS, %.2f:1 DSR, %.0f fails, %.2f:1 HRAll\n",
	...
	for(int i=0;i<shop_max;i++)
	{
		...
		if (v>0.05)
			LOG_V("profiler", "%s: %.2fM\n",shil_opcode_name(i),v);
	}


	if (prof.counters.shil.readm_reg!=0)
	{
		LOG_V("profiler", "***PROFILE REPORT***\n");
		prof.counters.print();
	}

	LOG_V("profiler", "opcode fallbacks:\n");
