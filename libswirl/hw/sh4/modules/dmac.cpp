...
InterruptID dmac_itr[] = { sh4_DMAC_DMTE0, sh4_DMAC_DMTE1, sh4_DMAC_DMTE2, sh4_DMAC_DMTE3 };
...
void WriteCHCR(u32 addr, u32 data)
{
    ...
    if (DMAC_CHCR(ch).TE == 0 && DMAC_CHCR(ch).DE && DMAC_DMAOR.DME)
    {
        if (DMAC_CHCR(ch).RS == 4)
        {
            ...
            printf("DMAC: Manual DMA ch:%d rs:%d src: %08X dst: %08X len: %08X SM: %d, DM: %d\n", ch, DMAC_CHCR(ch).RS, DMAC_SAR(ch), DMAC_DAR(ch), DMAC_DMATCR(ch), DMAC_CHCR(ch).SM, DMAC_CHCR(ch).DM);
            for (int ofs = 0; ofs < len; ofs += 4)
            {
                auto data = ReadMem32_nommu(DMAC_SAR(ch) + ofs);
