...
void X86NGenBackend::CC_Start(shil_opcode* op)
{
	ngen_CC_BytesPushed=0;
}
void X86NGenBackend::CC_Param(shil_opcode* op,shil_param* par,CanonicalParamType tp)
{
	switch(tp)
	{
		//push the contents
...
void X86NGenBackend::CC_Call(shil_opcode*op,void* function)
{
	reg.FreezeXMM();
	x86e->Emit(op_call,x86_ptr_imm(function));
	reg.ThawXMM();
}
void X86NGenBackend::CC_Finish(shil_opcode* op)
{
